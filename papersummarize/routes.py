...
from .models import Page, Paper, UserPaperRating, Tag, Tip, User
...
def includeme(config):
    ...
    config.add_route('add_user_paper_rating', '/x/{arxiv_id}/ratings/create/{rating}', factory=new_user_paper_rating_factory)
    config.add_route('edit_user_paper_rating', '/x/{arxiv_id}/ratings/edit/{rating}', factory=user_paper_rating_factory)
    config.add_route('delete_user_paper_rating', '/x/{arxiv_id}/ratings/delete', factory=user_paper_rating_factory)
    # User
...
def new_user_paper_rating_factory(request):
    arxiv_id = request.matchdict['arxiv_id']
    paper = request.dbsession.query(Paper).filter_by(arxiv_id=arxiv_id).first()
    if paper is None:
        raise HTTPNotFound
    return NewPaperRating(paper)
...
def user_paper_rating_factory(request):
    arxiv_id = request.matchdict['arxiv_id']
    paper = request.dbsession.query(Paper).filter_by(arxiv_id=arxiv_id).first()
    if paper is None:
        raise HTTPNotFound
    rating = request.dbsession.query(UserPaperRating).filter_by(creator=request.user, paper=paper).first()
    if rating is None:
        raise HTTPNotFound
    return UserPaperRatingResource(rating)

class UserPaperRatingResource(object):
    def __init__(self, user_paper_rating):
        self.user_paper_rating = user_paper_rating

    def __acl__(self):
        return [
            (Allow, Everyone, 'view'),
            (Allow, 'role:editor', 'edit'),
            (Allow, str(self.user_paper_rating.creator_id), 'edit'),
        ]
